...
export class RoleRepository extends BaseRepository<Role> {
  ...
  async findAll(
    ...
    const { sortKey, sortDir, search, ...searchOptions } = filterOptions;
    ...
    if (search) {
      queryBuilder.andWhere(`(LOWER(entity.name) LIKE :searchName)`, {
        searchName: `%${search.toLowerCase()}%`,
      });
    }
  ...
  async findAllByAccount(
    ...
    const { sortKey, sortDir, account, search, origin } = filterOptions;
    const { skip, limit } = paginationOptions;
    ...
    queryBuilder.andWhere(
      new Brackets((qb) => {
        qb.where('entity.origin = :origin', { origin: _origin }).orWhere(
          'entity.origin IS NULL',
        );
      }),
    );
  ...
  async findAllUsers(
    ...
    if (currentUser?.account?.type === AccountTypeEnum.AGENCY) {
      queryBuilder.innerJoin(
        ...
        {
          accountTypes: ['AGENCY', 'OPERATOR', 'COMPANY', 'AUDITOR'],
        },
      );
    } else {
      queryBuilder.innerJoin(
