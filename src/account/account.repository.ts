...
export class AccountRepository extends BaseRepository<Account> {
  ...
  async create(data, entityManager?: EntityManager): Promise<any> {
    ...
    if (account) {
      await this._handleWorkflowUserCreation(
        account.id,
        data.workflowGroups,
        entityManager,
      );
  ...
  private async _handleWorkflowUserCreation(
    accountId: number,
    workflowGroups: any,
    entityManager: EntityManager,
  ): Promise<void> {
    try {
      const accountWithUsers = await entityManager.findOne(Account, {
        where: { id: accountId },
        relations: ['users'],
      });

      const user = accountWithUsers?.users?.[0];

      if (user && !user.wfUserId) {
      ...
      this.loggerService.error(
        ...
        error,
        'AccountService.handleWorkflowUserCreation',
      );
    }
  }
  ...
  async findByUidOrSlug(identifier: string) {
    return this.accountRepository.findOne({
      where: [{ uuid: identifier }, { slug: identifier }],
    });
  }
  ...
  async findIndividuals(
    ...
    const { sortKey, sortDir, search } = filterOptions;
    const { skip, limit } = paginationOptions;
    ...
    queryBuilder
      ...
      .andWhere((qb) => {
        const subQuery = qb
          .subQuery()
          .select('staff.individualAccountId')
          .from('STAFF', 'staff')
          .where('staff.individualAccountId = account.id');
    ...
    if (search) {
      ...
      queryBuilder.andWhere(
        new Brackets((qb) => {
          qb.orWhere('LOWER(individual.firstName) LIKE :firstName', {
            firstName: searchKeyword,
          })
            .orWhere('LOWER(individual.lastName) LIKE :lastName', {
              lastName: searchKeyword,
            })
            .orWhere('LOWER(users.email) LIKE :email', {
              email: searchKeyword,
            });
        }),
      );
  ...
  async update(id: number, data): Promise<any> {
    ...
    switch (accountTypeRelation) {
      case AccountTypeEnum.INSTITUTION:
        repository = this.institutionRepository;
    ...
    const updatedData = {
      ...
      ...(data?.workflowGroups && {
        workflowGroups: StringHelper.stringify(data.workflowGroups),
      }),
    };
    await repository.update({ accountId: account.id }, updatedData);

    await this._synchronizeWfUserGroupsWithProcessMaker(id, data, account);

    return account;
  }

  private async _synchronizeWfUserGroupsWithProcessMaker(id, data, account) {
    if (data?.workflowGroups && Array.isArray(data?.workflowGroups)) {
      ...
      if (user) {
        ...
        let updatedUser;
        if (!user.wfUserId) {
          const refetchedAccount = await this.findById(id);
          updatedUser = refetchedAccount.users?.[0];
        } else {
          updatedUser = user;
        }

        if (updatedUser?.wfUserId) {
          try {
            const assignmentPromises = data.workflowGroups.map((group) =>
              this.workflowService.assignUserToGroup(
                updatedUser.wfUserId,
                group.id,
              ),
            );
          ...
          this.loggerService.warn(
            `User ${user.id} still has no wfUserId after sync.`,
          );
        }
      }
    }
  ...
  async generateUniqueSlug(
    ...
    while (taken) {
      ...
      const truncatedTitle = cleanedName.slice(0, maxTitleLength);

      slug = `${truncatedTitle}${suffix}`;
