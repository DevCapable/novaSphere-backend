...
const sluggables = [
  ...
  BaseRecordEnum.SERVICE_UTILIZATION_CATEGORY,
];
...
export class BaseRecordSeeder implements SeederInterface {
  ...
  async baseRecordSkeletonForResearchFocusArea() {
    if (await this.checkIfTypeExists(BaseRecordEnum.RESEARCH_FOCUS_AREA))
      return;
    const data = await convertCSVToObject(
      this.csvFilePath('research_focus_area.csv'),
    );
    data.map(async (data) => {
      // ADD THIS CHECK: Skip invalid names
      if (!data.name || data.name.trim() === '') return;
      ...
      if (category) {
        const record = this.baseRecord.create({
          ...
          isActive: 1,
          name: removeSpecialCharacters(data.name).toUpperCase().trim(),
          type: BaseRecordEnum.RESEARCH_FOCUS_AREA,
          parentId: category?.id,
        });
        await this.baseRecord.save(record);
      }
  ...
  async baseRecordSkeletonForDegree() {
    ...
    data.map(async (data) => {
      // ADD THIS CHECK: Skip invalid names (assuming 'name' is the field; adjust if it's 'degree' or similar)
      if (!data.name || data.name.trim() === '') return;
  ...
  async seed() {
    ...
    await this.baseRecordSkeleton(BaseRecordEnum.RESEARCH_CENTER_OF_EXCELLENCE);
    await this.baseRecordSkeletonForResearchFocusArea();
